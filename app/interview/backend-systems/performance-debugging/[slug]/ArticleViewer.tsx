...
export default function ArticleViewer({ slug, initialContent, topicPath }: ArticleViewerProps) {
  ...
  async function handleDelete() {
    const confirmed = window.confirm(
      'Are you sure you want to delete "' + title + '"?\n\nThis action will remove the article permanently from the directory.'
    );
    if (!confirmed) return;
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok && data.success) {
        router.push('/interview/' + topicPath);
      } else {
            ...
            {previewTab === 'edit' ? (
              <div>
                <textarea
                  ...
                  rows={24}
                  placeholder="Type markdown content here... (Tip: You can paste screenshots directly with Ctrl+V / Cmd+V or drag & drop image files!)"
                  style={{
                    width: '100%',
