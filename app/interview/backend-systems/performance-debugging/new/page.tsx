  ...
  async function uploadImageFile(file: File) {
    ...
    try {
      ...
      if (res.ok && json.url) {
        ...
        setUploadMessage('✓ Image inserted successfully!');
    ...
    try {
      ...
      const res = await fetch('/api/interview/create', {
        ...
        body: JSON.stringify({
          ...
          category,
          content: fullContent,
          topicPath: 'backend-systems/performance-debugging',
        }),
      });
      ...
      if (res.ok && data.success) {
        router.push('/interview/backend-systems/performance-debugging/' + data.slug);
      } else {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <Link href="/interview/backend-systems/performance-debugging" style={{ color: '#3b82f6', textDecoration: 'none', fontWeight: 500 }}>
          ← Back to Topic
        </Link>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
          <div>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.4rem' }}>Question Title *</label>
            <input type="text" required value={title} onChange={handleTitleChange} placeholder="e.g. How does garbage collection work?" style={{ width: '100%', padding: '0.6rem', borderRadius: '6px', border: '1px solid rgba(125,125,125,0.3)', background: 'transparent', color: 'inherit' }} />
          </div>
          <div>
            <label style={{ display: 'block', fontWeight: 600, marginBottom: '0.4rem' }}>URL Slug *</label>
            <input type="text" required value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="e.g. how-does-garbage-collection-work" style={{ width: '100%', padding: '0.6rem', borderRadius: '6px', border: '1px solid rgba(125,125,125,0.3)', background: 'transparent', color: 'inherit' }} />
          </div>
        </div>
        ...
        <div style={{ border: '1px solid rgba(125,125,125,0.25)', borderRadius: '8px', marginBottom: '1.5rem', overflow: 'hidden' }}>
          <div style={{ padding: '0.6rem 1rem', background: 'rgba(125,125,125,0.08)', borderBottom: '1px solid rgba(125,125,125,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              ...
              >
                {isUploading ? '⏳ Uploading...' : '🖼️ Insert Image'}
              </button>
              {uploadMessage ? (
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
          <Link href="/interview/backend-systems/performance-debugging" style={{ padding: '0.6rem 1.25rem', borderRadius: '6px', border: '1px solid rgba(125,125,125,0.3)', color: 'inherit', textDecoration: 'none' }}>
            Cancel
          </Link>
